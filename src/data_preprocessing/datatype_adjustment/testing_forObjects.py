...
def create_sample_data(file_path, sample_size=1000, random_seed=42):
    """Create sample dataset from the large CSV"""
...
def test_optimization_strategy(sample_df):
    """Test optimization strategy with validation"""
    ...
    try:
        ...
        if validation_passed:
            print("\nAll validations passed - transformation is safe!")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            optimized_sample.to_csv('sample_optimized.csv', index=False)
            print("\nOptimized sample saved to 'sample_optimized.csv' for inspection")
            print("Safe to proceed with full dataset")
            return True
        else:
